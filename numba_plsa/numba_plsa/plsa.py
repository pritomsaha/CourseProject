...
import time
from numba_plsa.plsa_basic import normalize_basic, plsa_basic
...
class PLSAModel(object):
  ...
  def train(self, doc_term, n_topics, n_iter, term_topic_prior, mu, min_count=1, method='basic'):
    self.topic_doc, self.term_topic = plsa(doc_term, n_topics, n_iter, term_topic_prior, mu, min_count, method)
...
def plsa(doc_term, n_topics, n_iter, term_topic_prior, mu, min_count=1, method='basic'):
    # Get size
    ...
    start = time.clock()
    if method == 'basic':
        topic_doc, term_topic = plsa_basic(doc_term, topic_doc, term_topic, n_iter, term_topic_prior=term_topic_prior, mu=mu)
    elif method == 'numba':
        plsa_numba(sp.row, sp.col, sp.data, topic_doc, term_topic, n_iter, term_topic_prior=term_topic_prior, mu=mu)
    else:
...
def plsa_direct(doc_term, n_topics, n_iter, term_topic_prior, mu):
    # Get size
    n_docs, n_terms = doc_term.shape
    ...
    start = time.clock()
    plsa_numba(doc_term.row, doc_term.col, doc_term.data, topic_doc, term_topic, n_iter, term_topic_prior=term_topic_prior, mu=mu)
    elapsed = time.clock() - start
    print ("\nRan {0} iterations in {1:.3f} seconds".format(n_iter, elapsed))
